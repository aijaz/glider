...
- [glider_left.png](https://github.com/aijaz/glider/blob/main/glider_left.png) <img alt="Glider Facing Left" src="glider_left.png">
- [glider_right.png](https://github.com/aijaz/glider/blob/main/glider_right.png) <img alt="Glider Facing Right" src="glider_right.png">
- [vent.png](https://github.com/aijaz/glider/blob/main/vent.png) ![Vent] <img alt="Vent" src="vent.png">
...
If you look at our three text views, you'll notice that the views are almost 
identical. We can take advantage of this fact by "refactoring" our views. We 
can "factor out" all the common stuff and introduce the differences when needed. 
It's easier to show this to you than explain it. 

What's common in our three views? 

- Each view contains 2 lines of text.
- Each view prompts us to press any key to start (or restart) the game

What's different in our three views?

- The text
- The background color

Let's replace all three views with a single view whose constructor (`__init__`)
takes in 3 additional parameters - the two lines of text, and the background color. 

Replace the 3 views you had above with the following:
```python
class TextView(arcade.View):
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        self.window.show_view(start_view)
```
Since we're using variables for `line1`, `line2`, and `bg_color` we don't need to 
have three almost identical classes.

Now we modify the places where we used the views. The first is in `main`. Change
...
def main():
    ...
    arcade.run()
```
Now, change `game_over` to look like this:
```python
    def game_over(self):
        ...
        self.window.show_view(game_over_view)
```
Finally, change `you_won` to look like this:
```python
    def you_won(self):
...
- Make new levels
- Experiment with making really hard levels. You can set the start level to that level, 
  so you don't need to play the entire game every time you want to get to the last level.
- Put coins in difficult-to-reach places. 
- Remove coins that are impossible to reach.
- Design your own graphics for this game. 
